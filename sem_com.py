...
def gpio_init():
    ...
    if user_conf.UDG1WSNE:
        #os.system('usb_modeswitch -v 0x0409 -p 0x04c7 --reset-usb')
        subprocess.call('/usr/bin/sudo /usr/sbin/usb_modeswitch -v 0x0409 -p 0x04c7 -R', shell=True)
    ...
    def run(self):
        while not self.term_flag:
            ...
            if msg:
                ...
                elif msg_list['COMMAND'] == 'ERXUDP' and msg_list['DATA'][0:4] == self.EHD \
        ...
        if y3.get_queue_size():     # データ受信
            ...
            if msg_list['COMMAND'] == 'ERXUDP':
                ...
                if parsed_data:
                    ...
                    sys.stdout.write('[Error]: ECHONET Lite frame error. @sem_get_getres\n')
                    return False
            else:
                sys.stdout.write('[Error]: Unknown data received. @sem_get_getres\n')
                return False
    ...
    while True:
        if y3.get_queue_size():     # データ受信
            ...
            if msg_list['COMMAND'] == 'ERXUDP':
                ...
                sys.stdout.write('[Error]: Unknown data received. @sem_seti\n')
                return False
...
if __name__ == '__main__':
    ...
    if sem_exist:
        ...
        for epc in get_list:    # 各種データ取得
            edt = False
            for i in range(10):
                sys.stdout.write('({}/10) {} --- '.format(i+1, epc))    # debug
                ...
                if data:
                    ...
                    if parsed_data:
                        ...
                        sys.stdout.write('[Error]: Get failed. @get_list_1\n')    # debug
                        time.sleep(10)
                        continue    # Get失敗　再試行
                else:   # Get失敗 再試行
                    sys.stdout.write('[Error]: Get failed. @get_list_2\n')        # debug
                    time.sleep(10)
                    continue
    ...
    if sem_exist:
        ...
        while True:
            try:
                # モジュールが自動で再認証(SKSREG S17)しているので定期更新は削除
                # GetRes待ちで PANA再認証フラグが立ったら再認証
                if not pana_done:
                    for i in range(10):
                        ...
                        if sem_exist:       # インスタンスリスト通知の受信待ち
                            ...
                            if pana_done:
                                break
                        else:   # restart_pac失敗 再試行
                            time.sleep(10)

                    if not pana_done:
                        sys.stdout.write('[Error]: Fail to re-connect PANA\n')    # debug
                ...
                while True:                             # 読み込み間隔調整
                    if (time.time() - start) >= user_conf.SEM_INTERVAL:
                        ...
                        if sem_inf_list:                # スマートメーターが自発的に発するプロパティ通知
                            msg_list_lock.acquire()
                            inf = sem_inf_list.pop(0)
                            msg_list_lock.release()
                            sys.stdout.write('[Inf]: {}\n'.format(inf['DATA']))
                ...
                if time.time() - time_period > 60:      # 1分間GetRes連続失敗
                    pana_done = False                   # PANA再認証フラグを立てる
                    errmsg = '[Error]: Continuous failure.\n'
                    sys.stdout.write(errmsg)
                    ...
                    while True:                         # GetRes待ち
                        ...
                        if y3.get_queue_size():
                            ...
                            if time.time() - start > 20:    # GetRes最大待ち時間: 20s
                                sys.stdout.write('[Error]: Time out for GetRes.\n')
                                break
                            time.sleep(0.1)
